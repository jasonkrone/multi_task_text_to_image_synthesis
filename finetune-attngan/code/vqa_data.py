...
def get_loader(train=False, val=False, test=False):
    ...
    split = VQA(
        ...
        config.train_path if train else config.val_path,
...
class VQA(data.Dataset):
    ...
    def __init__(self, questions_path, answers_path, images_path, answerable_only=False):
        ...
        self.images = CocoImages(images_path)
    ...
    def _load_image(self, image_id):
        """ Load an image """
        path = os.path.join(self.images.path,
                            self.images.id_to_filename[image_id])
        img = Image.open(path).convert('RGB')

        if self.transform is not None:
            img = self.transform(img)
        # return torch.from_numpy(np.moveaxis(np.array(img).astype('float32'), 0, -1))
